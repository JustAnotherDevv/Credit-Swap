...
const getAuthDomain = () => {
  return {
    name: process.env.APP_NAME, //"Nitro Aura",
  };
};
...
export class NitroliteRPCClient {
  ...
  async signMessage(data) {
    ...
    const message = {
      ...
      scope: process.env.APP_SCOPE, //"app.nitro.aura",
      wallet: address,
      application: address,
      participant: address,
      expire: expire,
    ...
    try {
      ...
      try {
        ...
        const { serialized: fallbackSignature } =
          this.wallet.signingKey.sign(messageBytes);
        ...
        throw new Error(
          `Both EIP-712 and fallback signing failed: ${eip712Error.message}`
        );
      }
    }
  }
  ...
  async authenticate() {
    ...
    return new Promise((resolve, reject) => {
      const authRequest = async () => {
        try {
          const request = await createAuthRequestMessage({
            ...
            app_name: process.env.APP_NAME, //"Nitro Aura",
            expire: expire,
            scope: process.env.APP_SCOPE, //"app.nitro.aura",
      ...
      const handleAuthResponse = async (data) => {
        try {
          logger.auth(
            `Received authentication response: ${data.slice(0, 100)}...`
          );
          ...
          if (response.res && response.res[1] === "auth_challenge") {
            ...
            logger.auth(
              `Sending auth verification: ${authVerify.slice(0, 100)}...`
            );
            ...
            try {
              ...
              if (!hasValidChannel) {
                logger.nitro(
                  "No valid channels found after authentication, will create one"
                );
              }
            } catch (error) {
              logger.error(
                "Failed to get channel info, continuing anyway:",
                error
              );
            }
  ...
  handleMessage(data) {
    try {
      ...
      if (
        message.err &&
        Array.isArray(message.err) &&
        message.err.length >= 3
      ) {
  ...
  async sendRequest(method, params = {}) {
    ...
    if (this.ws.readyState !== WebSocket.OPEN) {
      logger.error(
        `WebSocket not in OPEN state. Current state: ${this.ws.readyState}, Status: ${this.status}`
      );
      throw new Error(
        `WebSocket not in OPEN state. Current readyState: ${this.ws.readyState}`
      );
    }

    if (this.status !== WSStatus.CONNECTED) {
      logger.warn(
        `WebSocket status is ${this.status}, should be ${WSStatus.CONNECTED}. Proceeding anyway.`
      );
      if (this.status === WSStatus.AUTHENTICATING) {
        logger.system(
          "Fixing status to CONNECTED for authenticated connection"
        );
        this.setStatus(WSStatus.CONNECTED);
      }
    }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        logger.ws(
          `Sending request: ${JSON.stringify(signedRequest).slice(0, 100)}...`
        );
  ...
  async getChannelInfo() {
    try {
      ...
      const response = await this.sendRequest("get_channels", [
        { participant: this.address },
      ]);
...
export async function initializeRPCClient() {
  ...
  if (rpcClient) {
    logger.system(
      "Nitrolite RPC client already initialized, returning existing instance..."
    );
    return rpcClient;
  }
  ...
  try {
    ...
    rpcClient = new NitroliteRPCClient(process.env.WS_URL, SERVER_PRIVATE_KEY);
    ...
    rpcClient.walletClient = await getWalletClient(
