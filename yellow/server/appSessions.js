 ...
 */
export async function generateAppSessionMessage(
  roomId,
  participantA,
  participantB
) {
  try {
    ...
    logger.nitro(
      `Generating app session message for room ${roomId} with participants A: ${formattedParticipantA}, B: ${formattedParticipantB}`
    );
    ...
    if (pendingSession) {
      logger.nitro(
        `Using existing app session message for room ${roomId} - nonce: ${
          pendingSession.nonce
        }, requestToSign: ${JSON.stringify(pendingSession.requestToSign)}`
      );
      return {
        appSessionData: pendingSession.appSessionData,
        appDefinition: pendingSession.appDefinition,
        participants: [
          pendingSession.participantA,
          pendingSession.participantB,
    ...
    const appDefinition = {
      protocol: process.env.PROTOCOL_APP,
      participants: [
        formattedParticipantA,
        formattedParticipantB,
        serverAddress,
      ],
      weights: [0, 0, 100],
      quorum: 100,
    ...
    logger.data(
      `Generated request structure for room ${roomId}:`,
      requestToSign
    );
    ...
    logger.nitro(
      `App session message generated for room ${roomId} with nonce ${nonce}`
    );
    return {
      appSessionData,
      appDefinition,
    ...
    logger.error(
      `Error generating app session message for room ${roomId}:`,
      error
    );
    throw error;
  }
 ...
 */
export async function addAppSessionSignature(
  roomId,
  participantAddress,
  signature
) {
  try {
    ...
    const isValidParticipant = [
      pendingSession.participantA,
      pendingSession.participantB,
    ].includes(formattedParticipantAddress);
    if (!isValidParticipant) {
      throw new Error(
        `Invalid participant ${formattedParticipantAddress} for room ${roomId}`
      );
    }
    ...
    logger.nitro(
      `Added signature for ${formattedParticipantAddress} in room ${roomId} (${pendingSession.signatures.size}/2 collected)`
    );
    logger.data(`Signature details:`, {
      participantAddress: formattedParticipantAddress,
    ...
    const allParticipantsSigned =
      pendingSession.signatures.has(pendingSession.participantA) &&
...
export async function createAppSessionWithSignatures(roomId) {
  try {
    ...
    logger.nitro(
      `Creating app session with collected signatures for room ${roomId}`
    );
    ...
    const participantASignature = pendingSession.signatures.get(
      pendingSession.participantA
    );
    const participantBSignature = pendingSession.signatures.get(
      pendingSession.participantB
    );
    ...
    if (!participantASignature) {
      throw new Error(
        `Missing signature from participant A: ${pendingSession.participantA}`
      );
    }
    if (!participantBSignature) {
      throw new Error(
        `Missing signature from participant B: ${pendingSession.participantB}`
      );
    }
    ...
    logger.data(
      `Server signing request structure for room ${roomId}:`,
      pendingSession.requestToSign
    );
    ...
    logger.data(`Final message structure:`, {
      ...
      participantsOrder:
        pendingSession.appSessionData[0].definition.participants,
    ...
    logger.error(
      `Error creating app session with signatures for room ${roomId}:`,
      error
    );
    throw error;
  }
...
export async function createAppSession(roomId, participantA, participantB) {
  try {
    ...
    const signedMessage = await createAppSessionMessage(sign, [
      {
        definition: appDefinition,
        allocations: [
          {
    ...
    logger.data(
      `Signed app session message for room ${roomId}:`,
      signedMessage
    );
...
export async function closeAppSessionWithWinner(roomId, winnerId = null) {
  try {
    ...
    if (winnerId === "A") {
      ...
      logger.nitro(
        `Player A (${participantA}) wins room ${roomId} - taking full allocation`
      );
    } else if (winnerId === "B") {
      // Player B wins - gets all the funds
      allocations = ["0", "0.02", "0"]; // B gets both initial allocations
      logger.nitro(
        `Player B (${participantB}) wins room ${roomId} - taking full allocation`
      );
    } else {
    ...
    logger.error(
      `Error closing app session with winner for room ${roomId}:`,
      error
    );
    return false;
  }
...
export async function closeAppSession(roomId, allocations) {
  try {
    ...
    // Create the signed message
    const signedMessage = await createCloseAppSessionMessage(sign, [
      closeRequest,
    ]);

    logger.data(
      `Signed app session close message for room ${roomId}:`,
      signedMessage
    );
