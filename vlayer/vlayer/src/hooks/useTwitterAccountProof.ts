...
const REVOLUT_ACCOUNT_REDACTION_CONFIG = [
  {
    response: {
      json_body_except: [
        "user.firstName",
...
const REVOLUT_ACCOUNT_NO_PII_REDACTION_CONFIG = [
  {
    response: {
      json_body_except: ["user.id", "user.state"],
    },
  },
];
...
const steps = (
  redaction:
    | typeof REVOLUT_ACCOUNT_REDACTION_CONFIG
    | typeof REVOLUT_ACCOUNT_NO_PII_REDACTION_CONFIG
) => [
  startPage("https://app.revolut.com/start", "Go to revolut.com login page"),
  expectUrl("https://app.revolut.com/home", "Revolut home page"),
  notarize(
    "https://app.revolut.com/api/retail/user/current",
    "GET",
    "Generate Proof of Revolut account details",
    redaction
  ),
];
